...
pub fn benchmark(amount_of_iterations: i32) -> Result<(), Box<dyn Error>> {
    let implementation_variant = "armv8_bitsliced_implementation";
    ...
    let mut res_median_duration_expand_sk =
        format_duration_as_nanos(&final_median_duration_expand_sk); // Replace with format_duration(duration_expand_sk) when enabled
    let mut res_median_duration_expand_pk =
        format_duration_as_nanos(&final_median_duration_expand_pk);
    ...
    #[cfg(feature = "CCM1")]
    {
        ...
        res_median_duration_keygen = format_duration_as_string(
            &(cpu_speed_hz * (final_median_duration_keygen.as_nanos() as f64 / 1e9) as f64),
        );
        res_median_duration_expand_sk = format_duration_as_string(
            &(cpu_speed_hz * (final_median_duration_expand_sk.as_nanos() as f64 / 1e9) as f64),
        );
        res_median_duration_expand_pk = format_duration_as_string(
            &(cpu_speed_hz * (final_median_duration_expand_pk.as_nanos() as f64 / 1e9) as f64),
        );
        res_median_duration_sign = format_duration_as_string(
            &(cpu_speed_hz * (final_median_duration_sign.as_nanos() as f64 / 1e9) as f64),
        );
        res_median_duration_verify = format_duration_as_string(
            &(cpu_speed_hz * (final_median_duration_verify.as_nanos() as f64 / 1e9) as f64),
        );
    }
...
fn find_median(durations: &[Duration]) -> Duration {
