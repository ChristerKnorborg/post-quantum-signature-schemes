...
static inline void aes_setkey_encrypt(const unsigned char *key, uint8x16_t rkeys[]) {
    uint8x16_t key0 = vld1q_u8((const uint8_t *)(key + 0));
    ...
    #define BLOCK1(IMM)                                                          \
      temp1 = vaesimcq_u8(temp0);                                                \
      rkeys[idx++] = temp0;                                                      \
      temp4 = vextq_u8(temp0, vdupq_n_u8(0), 4);                                 \
      temp0 = veorq_u8(temp0, temp4);                                            \
      temp4 = vextq_u8(temp0, vdupq_n_u8(0), 8);                                 \
      temp0 = veorq_u8(temp0, temp4);                                            \
      temp1 = vsetq_lane_u8(rcon[IMM], temp1, 0);                                \
...
static void arm_aes128_ctr_enc_sch(const void *schedule, uint8_t *out,
                                        size_t out_len) {
    uint8x16_t mask = {8, 9, 10, 11, 12, 13, 14, 15, 7, 6, 5, 4, 3, 2, 1, 0};
    uint8x16_t block = vdupq_n_u8(0); // Initialize block to zero

    while (out_len >= 64) {
        uint8x16_t nv0 = block;
        uint8x16_t nv1 = vaddq_u8(vreinterpretq_u8_s64(vaddq_s64(vreinterpretq_s64_u8(vqtbl1q_u8(block, mask)), vdupq_n_s64(1))), mask);
        uint8x16_t nv2 = vaddq_u8(vreinterpretq_u8_s64(vaddq_s64(vreinterpretq_s64_u8(vqtbl1q_u8(block, mask)), vdupq_n_s64(2))), mask);
        uint8x16_t nv3 = vaddq_u8(vreinterpretq_u8_s64(vaddq_s64(vreinterpretq_s64_u8(vqtbl1q_u8(block, mask)), vdupq_n_s64(3))), mask);
        arm_aes128_encrypt_x4(schedule, nv0, nv1, nv2, nv3, out);
        block = vaddq_u8(vreinterpretq_u8_s64(vaddq_s64(vreinterpretq_s64_u8(vqtbl1q_u8(block, mask)), vdupq_n_s64(4))), mask);
        out += 64;
        out_len -= 64;
    }
    while (out_len >= 16) {
        ...
        out += 16;
        out_len -= 16;
        block = vaddq_u8(vreinterpretq_u8_s64(vaddq_s64(vreinterpretq_s64_u8(vqtbl1q_u8(block, mask)), vdupq_n_s64(1))), mask);
    }
    if (out_len > 0) {
        uint8_t tmp[16];
